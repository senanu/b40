...
dat <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vS7qlH_4s2a1BfKmpxCWgGg3KbPG95NSt6snRM_JtH7-BbKyKL1DcEePgRf3p4K4iyHUJhpdRhoyuf9/pub?output=csv")
```
...
```{r read_table2, echo=TRUE, eval=FALSE}
# Put your code here
x<-mydata
y<-mydata
mydata <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vS7qlH_4s2a1BfKmpxCWgGg3KbPG95NSt6snRM_JtH7-BbKyKL1DcEePgRf3p4K4iyHUJhpdRhoyuf9/pub?output=csv")
mydata
```
...
```{r summaryStats, echo=TRUE, eval=TRUE}
summary(mydata)
```
...
```{r myplot, include=TRUE, eval=TRUE}
plot(x = mydata     ,     # dat$length
     y =mydata      ,     # dat$width
     xlab="booklength",             # text within the quotes to label the x-axis
     ylab="bookwidth",             # text within the quotes to label the y-axis
     main="bookdimmentions")             # text within the quotes to label the whole graph
```
