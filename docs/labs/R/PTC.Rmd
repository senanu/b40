...
4. In the output (Fig \@ref(fig:orfs-tas-pic)), you should see a pictoral representation of the sequence that you submitted. Note that:
    ...
    a. select to see the six-frame translation track. You will see a picture similar to Fig \@ref(fig:orfs-pic).
4. In many cases, the longest ORF will be the one that codes for a protein. Right click the long ORF and select to "Set Sequence Origin At Feature", then accept the new origin of 85. This will just renumber the scale so that both are given.
...
1. You will see that the ORF has a codon "GCT" at that site and the variant converts the Cytosine to a Thymine.
1. You will also see that the codon GCT codes for Alanine (confirm this in the codon table (Fig \@ref(fig:codons-pic)), though you should realize that the DNA strand shown is the "coding" strand of DNA. The complementary strand to the "coding" strand is the "template" strand, and it is the template strand that produces the mRNA that is used in the codon table. Since the mRNA and the "coding" strand are both complementary to the same "template" strand, their sequences are the same except for the fact that RNA uses uracil instead of thymine.)
...
The polymerase chain reaction (PCR) makes many, many copies of a short region of DNA.

In order to study a region of DNA, it is often useful to make many copies of that region. Kary Mullis discovered that he could use use short DNA sequences to anneal to a section of DNA and then use a polymerase from bacteria to synthesize new DNA.
...
Please view a [complete animation of PCR here](https://www.bio-rad.com/webroot/web/movies/lse/global/english/what-is-polymerase-chain-reaction/tutorial.html).
...
1. Accept all the default values and then press "Get Primers" at the bottom.
1. Primer-BLAST will look for sequences that have particular characteristics such as
    ...
    c. the priming sequence doesn't exis elsewhere in the genome, or, if it does, there isn't a nearby site for the second primer to bind to.
1. Primer-BLAST will ask you about your PCR template being highly similar to some DNA in its database. Select the accession for TAS2R38 and click "Submit"
1. The results may take several minutes. Be patient.

When your results are returned to you, you should see something like \@ref(fig:primerBLASTOutput). There are several different primer pairs, but we will focus on the pair labeled "Primer 3". Note that your numbering may be different. This primer pair will amplify a region from 776 to 983. Note also the variant we have been interested in falls within this range (located at position 869).
...
knitr::include_graphics("pics/Fnu4H1.png")
```
...
1. Is the site that is in our PCR product there?
1. Save a scrieenshot. You'll turn this one in as well.
...
The results of our gel might look something like Fig (\@ref(fig:PTCGelPic)). We can thus distinguish between the alleles of a taster and a non-taster. Both of these gel lanes are what homozygotes would look like. A heterozygote would have both alleles. Re-sketch the gel showing a lane for a heterozygote.
...
1. A sketch of a gel similar to \@ref(fig:PTCGelPic) that includes the bands you'd expect for a heterozygote.
