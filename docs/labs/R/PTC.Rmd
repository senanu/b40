...
4. In the output (Fig \@ref(fig:orfs-tas-pic)), you should see a pictoral representation of the sequence that you submitted. Note that:
    ...
    a. select to see the six-frame translation track. You will see a picture similar to Fig \@ref(fig:orfs-pic). Zoom in to the picture using the 'magnifying glass' icon on the map.
...
1. On a piece of paper, sketch out the DNA that would result from your PCR. Label the left end 776 and the right end 964, as these are the coordinates of the end of the PCR product.
1. Write that the TAS2R38 variant is at base 869. This variant converts "GCTGC" (or "GCAGC" on the other strand) into "GTTGC". Write this sequence (GCTGC) in the correct spot and label it as the 'Taster' allele. Also indicate that the variant (non-taster allele) has the sequence GTTGC.
...
1. Notice that there are 2 sites that are within the range of our PCR product (776-964). In fact, one of them is at 868-872, exactly the spot where we have our TAS2R38 variant. Mark both of these sites on your paper map.
1. Save a screenshot. You'll turn this one in as well. It will be similar to \@ref(fig:Fnu4H1).
...
1. Annotate your hand-drawn map of the PCR product to show where the taster allele and the non-taster alleles would be cut by the restriction enzyme.
1. Take a picture of this map to turn in. You should have written on it where the DNA will be cur. Also write on it the expected size of fragments for both the taster and non-taster alleles.
...
1. A hand-drawn map of your PCR product, showing the sizes of both alleles.
